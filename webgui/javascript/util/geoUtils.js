...
class Trackpoint {
    ...
    calculateDistance(point) {
        ...
        let la2 = point.lat;
        let lo2 = point.lon;
        let latitude1R = (la1 * Math.PI) / 180.0;
        let longitude1R = (lo1 * Math.PI) / 180.0;
        let latitude2R = (la2 * Math.PI) / 180.0;
        let longitude2R = (lo2 * Math.PI) / 180.0;
        let dlat = latitude2R - latitude1R;
        let dlong = longitude2R - longitude1R;
    ...
    calculateBearing(point) {
        ...
        let y = Math.sin(longitude2R - longitude1R) * Math.cos(latitude2R);
        let x = Math.cos(latitude1R) * Math.sin(latitude2R) -
            Math.sin(latitude1R) * Math.cos(latitude2R) * Math.cos(longitude2R - longitude1R);

        let bearing = Math.atan2(y, x) / Math.PI * 180.0;
        while (bearing < -180.0) {
            bearing += 360.0;
        }
        while (bearing > 180.0) {
    ...
    xyPixelsFromStartPoint(startPoint, metersPerPixel) {
        let distance = startPoint.calculateDistance(new Trackpoint(this.lat, this.lon, 0, 0, 0));
        let bearing = startPoint.calculateBearing(new Trackpoint(this.lat, this.lon, 0, 0, 0)) - 90.0;
        let x = parseInt(Math.cos(bearing * Math.PI / 180.0) * distance / metersPerPixel);
        let y = parseInt(Math.sin(bearing * Math.PI / 180.0) * distance / metersPerPixel);
...
class Track {
    ...
    add(trackpoint) {
        ...
        if (this.points.length > 1) {
            let tp1 = this.points[this.points.length - 2];
            let tp2 = this.points[this.points.length - 1];

            let dist = tp1.calculateDistance(tp2);
            this.maxDist += dist;
        }
    }
